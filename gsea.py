...
class GSEA(object):
    ...
    def get_enrichment_score(self, gene_group: str):
        ...
        debug('gene group: {}'.format(gene_group))
        start0 = time.time()
        ...
        for gene in self.all_gene_sets[gene_group]:
            ...
            if gene.strip() in all_expression_genes:
                rel_group_genes.add(gene.strip())
    ...
    def get_sig_sets(self, p):
        ...
        for gene_group in self.gene_groups:
            ...
            for score in false_score_list:
                if score >= ES_score:
                    p_count += 1
            p_value = p_count / len(self.gene_groups)
...
def main():
    ...
    for g in list(gsea.all_gene_sets.keys()):
       gsea.get_enrichment_score(g)

    gsea.get_sig_sets(0.05)
